...
let ArticleTab = [{article : 42, author:'Fred'},{article : 1337, author:'Yuanqin'}];
ArticleTab = 'test';
...
const Article = ({ match }) => (
  <div>
    ...
    <Route path={`${match.url}/:ArticleId`} render={(props) => <ReadArticleProps articleId={props.ArticleId} articleTab={ArticleTab} />} />
  </div>
...
class ReadArticleProps extends Component {
  render() {
    return (
      <div>
      {match.params.ArticleId}
        {this.props.articleTab}
      </div>
    )
  }
}
